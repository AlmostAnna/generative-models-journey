...
def train_continuous(
    ...
    if plot_recon and scaler is not None:
        ...
        with torch.no_grad():
            ...
            # Inverse transform
            orig = scaler.inverse_transform(X_flat[0].cpu().numpy().reshape(1, -1)).reshape(16, 3)
            recon = scaler.inverse_transform(full_pred.cpu().numpy().reshape(1, -1)).reshape(16, 3)
            ...
            plt.legend(); plt.title("Training Reconstruction (Continuous)")
