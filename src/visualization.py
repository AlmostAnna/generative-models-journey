...
in the notebooks and some training scripts.
"""
...
def get_color_palette():
    """Return the configured color palette."""
    return colors
...
def plot_real_data(x_real, title="Two-Moons Dataset (Normalized)", save_path=None):
    ...
    plt.scatter(
        x_real[:, 0], x_real[:, 1], s=10, c=colors["real_data"], alpha=0.6, linewidth=0
    )
...
def plot_real_and_generated(real_data, gen_data, distr_title, real_title, gen_title):
    """Plot scatter plots for real and generated fata on specific axes."""
    ...
    ax2.scatter(
        gen_data[:, 0], gen_data[:, 1], s=5, c=colors["generated_data"], alpha=0.5
    )
...
def plot_comparison(data_2_plot, main_title=""):
    ...
    Args:
        data_2_plot (dict): A dictionary where keys are titles and values
                            are data arrays (e.g., torch tensors or numpy).
        main_title (str, optional): The main title for the entire figure.
                                    Defaults to "".
    """
    # Create figure and axes objects
    fig, axs = plt.subplots(
        ncols=len(data_2_plot), nrows=1, figsize=(12, 4), layout="constrained"
    )
    ...
    for ax, (title, samples) in zip(axs, data_2_plot.items()):
        ...
        if title == "Real Data":
            ...
            ax.scatter(
                x_data, y_data, s=5, c=get_color_palette()["real_data"], alpha=0.6
            )
        else:
...
def plot_true_contour_and_samples(true_samples, sampler_samples_dict, name=""):
    """Plot true data and samples with true data contour."""
    ...
    ax = axes[0]
    ax.contour(
        xx, yy, Z, levels=10, alpha=0.5, colors=colors["true_contour"], linewidths=0.5
    )
    ax.scatter(
        true_samples[:, 0],
        true_samples[:, 1],
...
def plot_chain_trajectories(trace, title="MCMC Chain Trajectories"):
    """Plot trajectories of the first 10 MCMC chains."""
...
def plot_energy_landscape_unified(
    model,
    data=None,
    ...
    if style in ["filled", "both"]:
        ...
        cs_fill = plt.contourf(
            X.numpy(), Y.numpy(), energies.numpy(), levels=50, cmap=colormap, alpha=0.8
        )
        if style == "filled":
    ...
    if style in ["lines", "both"]:
        ...
        cs_lines = plt.contour(  # noqa: F841
            X.numpy(),
            Y.numpy(),
            energies.numpy(),
            levels=levels,
...
def plot_real_vs_generated(
    ...
    if generated_data is not None:
        ...
        with torch.no_grad():
            x_gen = (
                ddim_sample(model, steps=50, n=n_samples, device=device).cpu().numpy()
            )
    else:
        raise ValueError(
            "Must provide either generated_data or model with sampling function"
        )

    # Create 2x2 plot
    fig = plt.figure(figsize=(12, 8))  # noqa: F841
    ...
    if loss_history and len(loss_history) > 1:
        plt.plot(
            step_history,
            loss_history,
            color=colors["loss_curve"],
            linewidth=2,
            label="Training Loss",
        )
        if step_history and loss_history:
            plt.scatter(
                step_history[-1], loss_history[-1], color=colors["loss_curve"], zorder=5
            )
...
def plot_loss_curve(
    loss_history, step_history, save_path=None, enforce_positive_y=False
):
