...
def compute_w1_distance(true_samples, sampler_samples, n=1000):
    ...
    for i in range(true_samples.shape[1]):
        w1 += wasserstein_distance(true_samples[:n, i], sampler_samples[:n, i])
    return w1
...
def plot_diagnostics_grid_az(sampler_samples_dict, max_lag=50, title=None):
    """Plot autocorrelation for each coordinate with arviz."""
    n_samplers = len(sampler_samples_dict)

    # Create a big figure with 2 columns (for x0, x1) and n_samplers rows
    fig, axes = plt.subplots(n_samplers, 2, figsize=(10, 3 * n_samplers), sharex=True)
    if n_samplers == 1:
    ...
    if title:
        ...
        )  # Adjust for both suptitle and xlabel
    else:
...
def plot_diagnostics_grid_np(sampler_samples_dict, max_lag=50, title=None):
    """Compute and plot autocorrelation with numpy."""
    n_samplers = len(sampler_samples_dict)

    # Create a big figure with 2 columns (for x0, x1) and n_samplers rows
    fig, axes = plt.subplots(n_samplers, 2, figsize=(10, 2.5 * n_samplers), sharex=True)
    if n_samplers == 1:
        axes = axes.reshape(1, -1)
